...
double Array::scalProdSelf()
{
	...
	int first = num_elements_to_send * rank;
	if (rank != ROOT_THREAD) first += rest;

	int last = num_elements_to_send * (rank+1) + rest; // last element exclusive
...
//	int num_elements_to_compute;
	...
	{
		MPI_Send(&ar[num_elements_to_send*rank + rest], num_elements_to_send, MPI_DOUBLE, ROOT_THREAD, SEND_TAG, MPI_COMM_WORLD);
	}
